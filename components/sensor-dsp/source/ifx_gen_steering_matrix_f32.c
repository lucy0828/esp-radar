...
void ifx_gen_steering_matrix_f32(float32_t ang_est_range, uint32_t num_angles,
                                 ...
                                 uint32_t num_ant, const matrix_instance_f32* pDst)
{
    assert(ang_est_range > 0.0f);
    assert(ang_est_range <= (M_PI / 2.0f)); // expected in radians
    assert(num_angles > 1);
    assert(num_ant > 1);
    assert(pDst != NULL);
    ...
    float32_t phase_calc_coefficient = -1.0f * M_PI * ((2.0f * antenna_spacing_mm) / lambda_mm);
    ...
    for (uint32_t kk = 0; kk < num_angles; ++kk)
    {
        ...
        {
            float32_t phase = phase_calc_coefficient * (float32_t)jj * sinf(angle);
            *pOut = cosf(phase);
            pOut++;
            *pOut = sinf(phase);
            pOut++;

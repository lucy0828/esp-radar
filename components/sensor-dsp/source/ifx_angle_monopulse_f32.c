...
int32_t ifx_angle_monopulse_f32(const cfloat32_t* rx1,
                                ...
                                float32_t* angle)
{
    ...
    const float32_t TWO_PI = (2.0F * M_PI);
    ...
    {
        rx1_ang = (uint32_t)atan2f(cimagf(rx1[i]), crealf(rx1[i]));
        rx2_ang = (uint32_t)atan2f(cimagf(rx2[i]), crealf(rx2[i]));
    ...
    return (status == 0U) ? IFX_SENSOR_DSP_STATUS_OK : IFX_SENSOR_DSP_ARGUMENT_ERROR;
}

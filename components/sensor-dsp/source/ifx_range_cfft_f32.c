                           ...
                           uint16_t num_chirps_per_frame)
{
    ...
    for (uint32_t chirp_idx = 0; chirp_idx < num_chirps_per_frame; ++chirp_idx)
    {
        ...
        if (win != NULL)
        {
            ...
            dsps_mul_f32(&((float32_t*) frame)[0], win, &((float32_t*) frame)[0], num_samples_per_chirp, 2, 1, 2);

            // apply to imag parts using step size of 2
            dsps_mul_f32(&((float32_t*) frame)[1], win, &((float32_t*) frame)[1], num_samples_per_chirp, 2, 1, 2);
        }

        dsps_fft2r_fc32((float32_t*) frame, num_samples_per_chirp);
        dsps_bit_rev2r_fc32((float32_t*) frame, num_samples_per_chirp);

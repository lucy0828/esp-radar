...
void ifx_window_blackmanharris_f32(float32_t* win, uint32_t len)
{
    ...
    for (uint32_t n = 0; n < len; ++n)
    {
        win[n] = BLACKMANHARRIS_CONST_A0 -
                 (BLACKMANHARRIS_CONST_A1 * cosf(2.0F * M_PI * (float32_t)n * M)) +
                 (BLACKMANHARRIS_CONST_A2 * cosf(4.0F * M_PI * (float32_t)n * M)) -
                 (BLACKMANHARRIS_CONST_A3 * cosf(6.0F * M_PI * (float32_t)n * M));
    }
}
